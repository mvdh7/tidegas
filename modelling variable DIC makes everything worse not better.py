...
def linear_fit_model(length, slope, start):
    #standard slope is per ml, so needs to be converted 
    slope_per_0_15mL = slope * 0.15/1
    return np.linspace(start, start+slope_per_0_15mL*length,length)
...
# ax.scatter(plot_df["acid added (mL)"]/1000,plot_df["Calculated DIC (umol/kg)"]+80, label = "DIC measurements + offset")
# ax.scatter(ttt.titrant_mass,co2s["dic"][0] +linear_fit_model(len(co2s["dic"]),slope,start), label = "improved DIC modelled using DIC loss" )
# test = (co2s["dic"][0]+linear_fit_model(len(co2s["dic"]),slope,start))
# ax.legend
