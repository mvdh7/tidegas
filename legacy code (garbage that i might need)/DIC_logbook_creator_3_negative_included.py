...
def remove_negative_slope(file_path):
    """
    ...
    corrected = np.array(corrected)[-1]
...
files_df["Negative slope corrected current [uAs]"] = [
    remove_negative_slope(os.path.join(folder_path, f))
    for f in files_df["DIC file name"]
]
...
def calc_dic_negative_removed(row):
    if pd.notna(row["Negative slope corrected current [uAs]"]) and pd.notna(row.get("Volume (mL)")):
        return float(row["Negative slope corrected current [uAs]"] / ((row["Volume (mL)"]/1000) * FARADAY))
    else:
        return None
